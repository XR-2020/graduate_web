    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-tickets"></i>软件著作权</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
...
<!--                </el-table-column>-->
            ...
            <router-link to="/合并申报">
                <el-button type="primary">软件著作权申报</el-button>
            </router-link>
        </div>
...
import {crawlerWebSite, editHeBing, getTeacherList} from "../../../api/commonAPI";
    export default {
        ...
        methods: {
            ...
            getData() {
                if(this.query.key!==''){
                    getSearchRuanJianZhuZuo(this.query).then(res =>{
                        this.tableData = res.list
                        this.pageTotal=res.pageTotal
                    } )
                }else{
                    getAllRuanJianZhuZuo(this.query).then(res=>{
                        this.tableData = res.list
                        this.pageTotal=res.pageTotal
                    })
                }
            ...
            handleEdit(index, row) {
                getRuanJianZhuZuoDetailBadge({id: row.id}).then(res =>{
                    this.form.people=res.data
                } )
                this.form=row;
                this.form.type='软件著作权';
                this.editVisible = true;
            },
            handleDetail(index, row){
                getRuanJianZhuZuoDetail({id: row.id}).then(res =>{
